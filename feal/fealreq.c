...
#if defined(USE_UDP) && !defined(__WIN32__)
...
static void dorequest(struct FealRequest *fr) {
	...
	if (s == -1) {
		...
		if (!(he = gethostbyname(FEALDAEMON_HOST))) {
			fprintf(stderr,
					"FATAL-ERROR: Can't unknown fealdaemon servr host " FEALDAEMON_HOST ".\n");
			exit(20);
		}
	...
	if (sendto(s, (char *) fr, sizeof(*fr), 0, (struct sockaddr *) &sin,
			sizeof(sin)) < 0) {
