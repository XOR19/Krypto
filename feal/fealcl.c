...
typedef cl_int (CL_API_CALL *clGetPlatformIDs_proc)
		(cl_uint          /* num_entries */,
        cl_platform_id * /* platforms */,
        cl_uint *        /* num_platforms */);

typedef cl_int (CL_API_CALL *clGetDeviceIDs_proc)
...
typedef cl_int (CL_API_CALL *clBuildProgram_proc)
		(cl_program           /* program */,
        cl_uint              /* num_devices */,
...
typedef cl_kernel (CL_API_CALL *clCreateKernel_proc)
		(cl_program      /* program */,
        const char *    /* kernel_name */,
        cl_int *        /* errcode_ret */);

typedef cl_mem (CL_API_CALL *clCreateBuffer_proc)
		(cl_context   /* context */,
        cl_mem_flags /* flags */,
        size_t       /* size */,
        void *       /* host_ptr */,
...
typedef cl_int (CL_API_CALL *clSetKernelArg_proc)
		(cl_kernel    /* kernel */,
        cl_uint      /* arg_index */,
        size_t       /* arg_size */,
        const void * /* arg_value */);

typedef cl_int (CL_API_CALL *clFlush_proc)
		(cl_command_queue /* command_queue */);

typedef cl_int (CL_API_CALL *clFinish_proc)
		(cl_command_queue /* command_queue */);

typedef cl_int (CL_API_CALL *clReleaseKernel_proc)
		(cl_kernel   /* kernel */);

typedef cl_int (CL_API_CALL *clReleaseProgram_proc)
		(cl_program /* program */);

typedef cl_int (CL_API_CALL *clReleaseMemObject_proc)
		(cl_mem /* memobj */);

typedef cl_int (CL_API_CALL *clReleaseCommandQueue_proc)
		(cl_command_queue /* command_queue */);

typedef cl_int (CL_API_CALL *clReleaseContext_proc)
		(cl_context /* context */);
...
feal_cl_state create_feal_cl(){
	...
	ret = CL.GetDeviceIDs(platform_id, CL_DEVICE_TYPE_DEFAULT, 1, &state->device_id, &ret_num_devices);

	/* Create OpenCL Context */
	state->context = CL.CreateContext(NULL, 1, &state->device_id, NULL, NULL, &ret);

	/* Create command queue */
	state->command_queue = CL.CreateCommandQueue(state->context, state->device_id, 0, &ret);

	/* Create kernel program from source file*/
	state->program = CL.CreateProgramWithSource(state->context, 1, &CL_SRC, NULL, &ret);
	ret = CL.BuildProgram(state->program, 1, &state->device_id, NULL, NULL, NULL);

	/* Create data parallel OpenCL kernel */
	state->kernel = CL.CreateKernel(state->program, "t", &ret);

	state->pairs_size = INIT_SIZE;
	state->pairs_clobj = CL.CreateBuffer(state->context, CL_MEM_READ_ONLY, INIT_SIZE*sizeof(cl_uchar4), NULL, &ret);
	state->num_outs_clpbj = CL.CreateBuffer(state->context, CL_MEM_READ_WRITE, sizeof(cl_int), NULL, &ret);
	state->out_size = INIT_SIZE;
	state->out_clpbj = CL.CreateBuffer(state->context, CL_MEM_WRITE_ONLY, INIT_SIZE*sizeof(cl_uchar4), NULL, &ret);

	ret = CL.SetKernelArg(state->kernel, 1, sizeof(cl_mem), (void *)&state->pairs_clobj);
	ret = CL.SetKernelArg(state->kernel, 3, sizeof(cl_mem), (void *)&state->num_outs_clpbj);
	ret = CL.SetKernelArg(state->kernel, 4, sizeof(cl_mem), (void *)&state->out_clpbj);
...
feal_cl_size_t feal_cl_generate_keys(feal_cl_state state,feal_cl_size_t num_pairs,feal_cl_plaintext_pair* pairs,feal_cl_size_t num_outs,feal_cl_key_pair* outs){
	cl_int ret;
	cl_int num_out = 0;
	int i;
	...
	if(num_pairs>state->pairs_size){
		...
		state->pairs_clobj = CL.CreateBuffer(state->context, CL_MEM_READ_ONLY, num_pairs*sizeof(cl_uchar4), NULL, &ret);
		ret = CL.SetKernelArg(state->kernel, 1, sizeof(cl_mem), (void *)&state->pairs_clobj);
	}
	...
	if(num_outs>state->out_size){
		...
		state->out_clpbj = CL.CreateBuffer(state->context, CL_MEM_WRITE_ONLY, num_outs*sizeof(cl_uchar4), NULL, &ret);
		ret = CL.SetKernelArg(state->kernel, 4, sizeof(cl_mem), (void *)&state->out_clpbj);
	}
	...
	ret = CL.EnqueueWriteBuffer(state->command_queue, state->pairs_clobj, CL_FALSE, 0, num_pairs*sizeof(cl_uchar4), state->buffer, 0, NULL, NULL);

	ret = CL.EnqueueWriteBuffer(state->command_queue, state->num_outs_clpbj, CL_FALSE, 0, sizeof(cl_int), &num_out, 0, NULL, NULL);

	ret = CL.SetKernelArg(state->kernel, 0, sizeof(cl_int), (void *)&num_pairs);
	...
	ret = CL.EnqueueReadBuffer(state->command_queue, state->num_outs_clpbj, CL_TRUE, 0, sizeof(cl_int), &num_out, 0, NULL, NULL);
	max = num_out;
	if(max>num_outs)
		max = num_outs;
	ret = CL.EnqueueReadBuffer(state->command_queue, state->out_clpbj, CL_TRUE, 0, max*sizeof(cl_uchar4), state->buffer, 0, NULL, NULL);
