...
static void getBit(ubyte(*Feal_Gs)(ubyte,ubyte), feal_cl_key_pair* keys, ubyte bit){
	ubyte mask = 1<<bit;
...
static feal_cl_size_t choosen_plaintext_attack(feal_cl_size_t num_keys, feal_cl_key_pair* keys){
	if(num_keys<=0)
		return 4;
	keys->k1 = 0;
	keys->k2 = 0;
...
static feal_cl_size_t known_plaintext_attack_soft(feal_cl_size_t num_pairs, feal_cl_plaintext_pair* pairs, feal_cl_size_t num_keys, feal_cl_key_pair* keys){
	...
	for(i=0; i<num_proc; i++){
		...
		pthread_create(&thread_datas[i].thread, NULL, known_plaintext_attack_thread, (void*)&thread_datas[i]);
	}
	if(t<0x1000000){
...
static feal_cl_size_t known_plaintext_attack(feal_cl_size_t num_pairs, feal_cl_plaintext_pair* pairs, feal_cl_size_t num_keys, feal_cl_key_pair* keys){
	feal_cl_state cl_state = create_feal_cl();
	if(cl_state){
		printf("Using OpenCL\n");
		feal_cl_size_t max_keys = feal_cl_generate_keys(cl_state, num_pairs, pairs, num_keys, keys);
