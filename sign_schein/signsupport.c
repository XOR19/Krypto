...
void Generate_MDC(const Message *msg, mpz_t p, mpz_t mdc) {
	...
	default:
		...
		exit(20); // here also better to return an error code...
		break;
	}
...
int Get_Private_Key(const char *filename, mpz_t p, mpz_t w, mpz_t x) {
	...
	*bufsize = 0;
	...
	if (!filename)
		filename = concatstrings(getenv("HOME"), "/private_key.data", NULL);
	if (!(f = fopen(filename, "r"))) {
		fprintf(stderr, "GET_PRIVATE_KEY: Kann die Datei %s nicht öffnen: %s\n", filename, strerror(errno));
		return 0;
	}
	if (getline(&line, bufsize, f) <= 0 || mpz_set_str(p, line, 16) || getline(&line, bufsize, f) <= 0
			|| mpz_set_str(w, line, 16) || getline(&line, bufsize, f) <= 0 || mpz_set_str(x, line, 16)) {
