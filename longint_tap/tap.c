...
int main(int argc, char **argv) {
	...
	DisConnect(con);
	name1 = (char*)"cr4ck1411_abu";
	name2 = (char*)"cr4ck1411_abd";
		...
		if (cnt == sizeof(pkt)) {
			...
			if (!is_valid_packet(&pkt)) {
				...
				if (pkt.tp == PACKETTYPE_Auth) {
					...
					if (pkt.number[0] == '1' && pkt.number[1] == '8'
							&& pkt.number[2] == '1') {
						pkt.number[strlen(
								"18171a0225e2aed352413e3ebe172d8f23d234a7edfeca829f0b0b2d9028a22")] =
								0;
					} else if (pkt.number[0] == 'a' && pkt.number[1] == 'a'
							&& pkt.number[2] == '6') {
						pkt.number[strlen(
								"aa6aef6638ee9bab065d40960687a60f9cab9c946d7391fa0524cc53dddaad0")] =
								0;
					} else if (pkt.number[0] == 'b' && pkt.number[1] == '2'
							&& pkt.number[2] == 'e') {
						pkt.number[strlen(
								"b2eee7553429ea891c2aa4405704fc0ba2c35bd563e8334aea3397b23791a54")] =
								0;
					...
					doexp(pkt_num, pkt.direction == DIRECTION_AliceBob ? b : a,
							pkt_num, p);
